...
namespace GameBase.Template.Item.GameBaseItem.Common
{
    ...
    {
        private void _Run_LoadUser_DBItem(AdoDB adoDB, UInt64 user_db_key, UInt64 player_db_key)
        {
            try
            {
                ...
                while (adoDB.RecordWhileNotEOF())
                {
                    ...
                    slot._DBData.remain_charge_time = adoDB.RecordGetValue("remain_charge_time");
                }
                adoDB.RecordEnd();
            }
            catch (Exception e)
        ...
        {
            _Run_LoadUser_DBItem(adoDB, partitionKey_1, partitionKey_2);
        }
    }
}
